...
| `shell` | 001 §D's three measurements verbatim: opposing pairs, ray-cast inner/outer discrimination, one t everywhere; **plus** 001 §D's opening identification — the removed-face set resolves to exactly the openings' stations and footprints (`shell-opening-unidentified` cases, fully enclosed voids included, are unconstructible here exactly as the executor refuses them); competes in the search against the equivalent explicit interior cuts (§6 decides; 001 §D's fallback semantics unchanged) |
