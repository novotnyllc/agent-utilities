...
- **Execution profile:** Goal Driven Delivery owns one host-local lane. Task Orchestrator owns multi-lane, cross-project, cross-machine, and future cross-account coordination.
...
- **Tail ownership:** LFG owns plan-through-CI delivery. Goal Driven Delivery owns the authorized merge and post-merge tail. Task Orchestrator verifies each delegated lane and the integrated outcome.
...
Goal Driven Delivery becomes an intent router and completion policy over Compound Engineering. Task Orchestrator remains the higher multi-lane control plane and propagates the lane policy without implementing work itself.
...
The two orchestration layers need distinct, durable integration behavior. Lane work must be resumable across agents and machines through writable GitHub branches before PR readiness, while the orchestrator must retain project-level integration ownership and dependency visibility.
...
- R8. Let Task Orchestrator own the global concurrency and host allocation budget, use Sol High or Max for its own orchestration, and propagate the Goal Driven Delivery model and completion policy to each lane.
...
- KTD7. **The orchestrator stays a control plane.** It owns decomposition, placement, integration ownership, global concurrency, monitoring, and terminal evidence. Each implementation lane invokes Goal Driven Delivery and never sends execution back into the orchestrator.
...
flowchart TB
  ...
  Multi -->|yes| Orchestrator["Task Orchestrator: Sol High or Max"]
  Orchestrator --> Lane["Owned Goal Driven Delivery lane"]
  Multi -->|no| Lane
...
### U2. Align Task Orchestrator with the lane policy

- **Goal:** Preserve the orchestrator's control-plane boundary while adding Sol orchestration, global concurrency, integration-branch, checkpoint, stack, and lane-policy propagation rules.
- **Requirements:** R7-R12, R14-R16.
- **Dependencies:** U1.
- **Files:** `plugins/agent-utilities/skills/task-orchestrator/SKILL.md`, `plugins/agent-utilities/skills/task-orchestrator/agents/openai.yaml`.
- **Approach:** Extend the existing model table and cross-project integration ownership instead of adding a second execution path. Add the same conditional GitHub `gh-stack` bootstrap at the orchestrator preflight. Keep implementation, testing, commits, pushes, and merges delegated to owned lanes.
- **Patterns to follow:** Existing canonical-writer, host readiness, task ledger, dependency, evidence, and cleanup contracts.
- **Test scenarios:**
  - A single local lane bypasses the orchestrator and enters Goal Driven Delivery.
...
- **Verification:** The orchestrator never claims lane execution, and every delegated implementation lane inherits the model, integration, and completion policy.
...
- **Test scenarios:**
  ...
  - Documentation distinguishes orchestrator coordination from lane execution and distinguishes pushed checkpoints from review-ready PRs.
...
- **Requirements:** R1-R16.
- **Dependencies:** U1-U3.
- **Files:** `plugins/agent-utilities/skills/goal-driven-delivery/SKILL.md`, `plugins/agent-utilities/skills/task-orchestrator/SKILL.md`, `docs/delivery-workflows.md`, `AGENTS.md`.
...
| Integration lifecycle review | Orchestrator, GDD, docs | Checkpoint push, PR readiness, stack, merge, and post-merge proof are distinct |
| Model-policy review | Orchestrator, GDD, docs | Sol, Luna, and Fable roles and effort escalation agree everywhere |
...
- Task Orchestrator remains the multi-lane control plane and propagates the lane model, integration, and completion policy.
