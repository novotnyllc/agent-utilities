...
- **No new dependency, and the reason is empirical, not conservative** (KTD2): `Shpigford/nurb` ships OpenCASCADE 7.9, numpy, scipy and trimesh, and *still* cannot deliver an editable CAD timeline — its exports are STEP and mesh, and its only parametric artifact is a Python source file. A second B-Rep kernel does not produce a Fusion timeline. Fusion's API does. The dependency question and the packaging question are answered separately and in full.
...
**F4.** Fusion's embedded Python **cannot import numpy.** `references/mcp-adapter.md:50–58` records a live probe of the connected MCP session: `os`, `tempfile`, `hashlib`, `uuid` imported; `secrets`, `sqlite3`, `numpy`, `requests` did not. The same file's guidance: *"Put heavy mesh or other non-Fusion processing in the host environment and pass only its result into Fusion."*

**F5.** The module-bundle cache **explicitly excludes native extensions.** `references/mcp-adapter.md:86–88`: *"Only regular `.py` files are supported; data files, symlinks, hard links, package-manager installation, and compiled/native extensions are intentionally excluded."* There is therefore no mechanism to get a compiled dependency into Fusion's interpreter.
...
- **R2. A declared triangle budget, refused rather than exceeded.** The caller declares `max_triangles` with a rationale. Above it the extraction refuses with `triangle-budget-exceeded`, naming the count and the budget, and offering decimation as the alternative. No silent multi-minute run.
...
- **R17. Runtime capabilities are probed live and recorded, never assumed from documentation.** A read-only probe transaction records, against the connected Fusion version, which of `struct`, `os`, `tempfile`, `hashlib` and `numpy` import in the embedded interpreter, and which of the mesh and construction APIs this plan needs are present. The result is written into the evidence chain and referenced by every downstream record — not into a doc comment. `references/mcp-adapter.md`'s recorded probe (F4) is one session on one installation and is treated as a prior, not a fact. *(Settles A1 and A4 as a by-product.)*
...
**Why numpy/scipy is tempting, where it would genuinely help, and why it still waits.** Being accurate about this rather than dismissive: numpy would *not* meaningfully help the fitting math — the eigen-decompositions are 3×3 and the least-squares systems are 4×4, where numpy's call overhead roughly cancels its advantage. Where it **would** help is the per-triangle work in U1/U2: normals, areas, edge adjacency and dihedral angles over every triangle. Those are tight Python loops and numpy would vectorize them perhaps 20–50×.

The honest estimate (A5, to be measured, not asserted): at 200k triangles, per-triangle arithmetic at roughly 30 float operations each is on the order of 3–8 seconds in CPython; adjacency construction is ~600k dict operations, 1–2 seconds; region growing about a second. **Tens of seconds for a 200k-triangle mesh**, in a workflow where the user is going to read the fit record before anything is emitted anyway. That is acceptable. Fit-driven splitting is the unbounded risk, and R15's declared recursion depth bounds it. Above the declared budget we decimate or refuse (R2) rather than run indefinitely.

So numpy buys **speed, not capability**, on a stage whose speed is already adequate. That is not enough to justify changing how the skill installs.

**Question 1 the coordinator asked: is host-side dependency in scope for the skill's packaging? Yes as a question, no as part of this feature — and the two packaging problems have genuinely different answers.**

*(a) In-Fusion code — the module bundle should stay `.py`-only. Do not grow it to carry wheels.* Its `.py`-only rule, its content-addressed verification, and its single hand-authored package are the reasons it is auditable. Growing it into a dependency mechanism means reimplementing pip's resolution, transitive closure, version pinning and wheel unpacking **inside a security-sensitive cache that is POSIX-only by design** (open issue #19). That is a large project defending a small win, and the win is illusory: the libraries anyone would actually want in Fusion — numpy, scipy, trimesh, OCCT — are all native, so a pure-Python wheel carrier would not carry any of them. **Recommendation: state "the module bundle is a code-sharing mechanism for our own pure-Python code, not a dependency mechanism" as an explicit non-goal in `references/unsupported.md`.** It is a correct design; it should be defended, not extended.

*(b) Host-side dependencies — ordinary Python packaging, as a separate small PR, and not a prerequisite for this feature.* The user has said they want a proper mechanism, and they should have one; it just is not on this feature's critical path. The correct shape, when it is built:
...
**Question 2: do not treat the numpy evidence as settled — re-probe it.** F4 is one live session on one installation, and `references/mcp-adapter.md` is careful to scope it that way. U0 adds an explicit capability-probe transaction that records, against the connected Fusion version, whether `numpy`, `struct`, `os`, `tempfile` and `hashlib` import — written into the evidence chain rather than into a doc comment.

**What changes if numpy *does* load in the user's Fusion: less than it looks.** The architecture reason for host-side numerics is not numpy's availability — it is that host-side code is **testable offline under `scripts/test.sh` with no Fusion running**, which is worth more than any speedup and is why U2 (the riskiest unit) can be fully proven before a single live call. That reason is unaffected by the probe. What the probe would unlock is narrower and real: the per-triangle arithmetic could move *into* the extraction transaction, so the wire carries regions instead of raw triangles and the dump shrinks by an order of magnitude. That is a worthwhile optimization to hold in reserve. It is not a reason to restructure, and it must not become one, because it would trade offline testability for speed.

**What would overrule me.** If the user's real corpus is routinely 500k+ triangles and decimation destroys the features they care about, the stdlib path becomes too slow and the optional host-side numpy extra becomes necessary. That is a measurement, and U2's benchmark is where it gets made. What would *not* change my mind at any measurement is OCCT.
...
**Goal:** Replace an assumption with a recorded measurement, before anything is designed around it.

**Requirements:** R17. Assumptions settled or narrowed: A1, A4, and the numpy question in KTD2.

**Files:** `src/fusion_design/mesh_probe.py` (new), `src/fusion_design/cli.py`, `references/mcp-adapter.md`, `tests/test_mesh_probe.py`.
...
- `Application.version`;
- per-module import results for `struct`, `os`, `tempfile`, `hashlib`, **`numpy`** — each attempted in its own `try` and reported as `available` / `unavailable` with the exception text, never as a bare boolean;
...
The report is a probe record with the Fusion version as its key, retained as evidence and referenced by every downstream artifact. It creates nothing and modifies nothing.

**Why this is its own unit rather than a line in U1:** it is the only thing in the plan whose entire purpose is to convert assumptions into facts, and it must land *first* so U1's and U5's designs can respond to the answers rather than to F4's single prior session. It is also the cheapest possible live run — no geometry, no risk — which makes it the right first thing to ask a user to execute.

**What changes on each outcome, decided now so the result is not re-litigated later:**

| Probe result | Response |
| --- | --- |
| `numpy` unavailable (expected, matches F4) | Nothing changes. Proceed as planned. |
| `numpy` available | Record it; change nothing structurally (KTD2). Note the optional future optimization of moving per-triangle arithmetic into the extraction transaction. Do **not** take it in this plan — it would trade offline testability for speed. |
| Face groups are one group or absent (A1 likely) | U2's own segmentation is the primary path, not the fallback. The agreement statistic is simply unavailable and is reported so. |
...
| --- | --- | --- | --- |
| 1 | U0 + U1 | S + M | Yes — a recorded capability probe against the user's actual Fusion, and `emit-mesh-extract` to pull a mesh out and inspect its face groups and noise floor host-side. Settles A1, A4, and the numpy question. U0 lands first within the PR so U1's fallbacks respond to real answers. |
...
10. **numpy or scipy inside Fusion's interpreter.** Structurally blocked: numpy did not import in the recorded live session (F4) and the module bundle mechanism excludes native extensions by design (F5) — and *even if U0's probe finds numpy present*, the bundle still cannot deliver it to another machine, so it could never be depended on. Host-side numpy remains available as an *optional speed-only* extra if U2's benchmark demands it (KTD2), never as a correctness dependency.

11. **Growing the module bundle into a dependency mechanism.** Declared a **non-goal**, to be written into `references/unsupported.md` in U6. Making it carry wheels means reimplementing pip's resolution, transitive closure, version pinning and wheel unpacking inside a security-sensitive, content-addressed, POSIX-only cache (open issue #19) — and it would still carry none of the libraries anyone actually wants, because numpy, scipy, trimesh and OCCT are all native. It is a code-sharing mechanism for our own pure-Python code. That is a correct design and it should be defended rather than extended. Host-side dependencies, if ever wanted, belong in ordinary Python packaging (KTD2), which is a different problem with a different answer.
