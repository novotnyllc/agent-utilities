...
- **R6.** Per part: `material` `{assumption: non-empty string (e.g. "PETG"), source_id?: declared manifest source, status: provisional|coupon-verified}` — links geometry-relevant material assumptions to an explicit decision; the full material gate is issue #21, which will build on this field.
