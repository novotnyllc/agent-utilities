...
**[Amended 2026-08-21, coordinated with 003 Amendment §7.3: when the causal-E influence maps land, `interactions_exercised` becomes `false | <exercised fraction of declared interaction cases> | unmeasured-no-declared-cases`. The schema here, the host-side validator's accepted values, and the U5 test scenarios are amended in that same implementation PR, in lockstep — until it lands, the literal `interactions_exercised: false` contract above stands and is the only value the shipped validator accepts.]**
