...
- R16. Codex saved-project registration must use a supported product capability when one exists; otherwise it must return the exact owner UI action and never edit Codex internal databases.
- R17. Saved-project IDs and visible task IDs must be freshly discovered and bound immediately before task creation or a work-starting follow-up.
...
- F2. Project preparation and dispatch
  ...
  - **Actors:** A2, A3, A5, A6.
  - **Steps:** A5 plans and verifies the exact safe action; A1 or standing policy approves required mutation; A5 applies and re-inventories; A2 creates the immutable writer-attempt reservation from post-inventory facts; A6 freshly resolves the saved project and pre-create account attestation; A2 claims inert task creation; A6 persists the canonical post-create task/account binding without a model turn; A2 validates it and invokes the fixed activation adapter with a distinct `maxTaskCount: 1` authority and reserved start budget.
...
- AE5. **Covers R16-R17.** Given a valid checkout with no saved project and no supported registration API, when dispatch is requested, then the run pauses with the exact owner registration action and no Codex database write.
...
| Post-create dispatch binding | reservation and account-attestation digests, freshly resolved saved project, inert task/correlation IDs, canonical task-to-account binding, activation authority, provider/model, executor manifest digest |
...
- **Approach:**
  ...
  3. Immediately before inert create, the destination runtime freshly discovers the saved project and returns the KTD10 pre-create single- or multi-account attestation. Build the R52 projection and route through the fixed host-owned in-process embedding. Missing readiness, authority attestor, native receipt importer, or account attestation returns the released typed unavailable result.
  4. Invoke the U1-proven inert/idempotent create, append the canonical post-create task/account binding, and validate objective/project/account/baseline/scope. Then invoke only KTD16's fixed activation adapter with its separately claimed `maxTaskCount: 1` authority, reserved start budget, current epoch/fencing, and native settlement. Never use a second `task_create`, public-CLI importer, or unrouted native task as activation.
  5. Missing registration/login returns the exact owner action. A lost create response uses only the proven exact reconciliation primitive; otherwise the lane remains owner-resolved quarantine.
