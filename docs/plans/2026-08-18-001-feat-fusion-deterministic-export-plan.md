...
- **R2.** Export STEP plus 3MF and/or STL per requested formats through `design.exportManager`. Export scope per format: STEP exports the resolved root-context occurrence; 3MF/STL export the resolved single solid body — R1's single-solid gate makes the two scopes equivalent in content, and the index records `export_scope` (`occurrence` | `body`) per artifact. If the export manager or a required option constructor is unavailable, emit an explicit unsupported/fail-closed report naming the missing attribute (e.g. `createC3MFExportOptions` — note the `C` prefix; a leading digit is not a legal identifier) — never a silent skip.
