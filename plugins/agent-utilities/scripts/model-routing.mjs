...
export const CONTRACT_VERSION = "agent-utilities/model-routing/v1";
export const CATALOG_SCHEMA_VERSION = 1;
export const STATE_SCHEMA_VERSION = 4;
export const STATE_PURPOSE = "agent-utilities/model-routing-state";
...
const TRUSTED_RECEIPT_IMPORTER_ID = "agent-utilities-adapter-receipt-importer-v1";
...
const ACTION_RECEIPT_SCHEMA = "agent-utilities/action-receipt/v1";
const INVARIANT_WORK_CONTRACT_SCHEMA = "agent-utilities/invariant-work-contract/v1";
const R52_READINESS_SCHEMA = "agent-utilities/r52-readiness/v1";
...
const FIXED_LOCAL_PROBE_ATTESTOR = "agent-utilities-fixed-local-probe-v1";
const HOST_CAPABILITY_ATTESTOR = "agent-utilities-host-attestor-v1";
const ADAPTER_RECEIPT_ATTESTOR = "agent-utilities-adapter-receipt-attestor-v1";
const TASK_AUTHORITY_ATTESTOR = "agent-utilities-task-authority-attestor-v1";
const TRANSPORT_ATTESTOR = "agent-utilities-transport-attestor-v1";
const RUNTIME_ATTESTOR = "agent-utilities-runtime-attestor-v1";
const CE_SEAMS = freeze({
  "ce-plan.execution": freeze({ skill: "ce-plan", artifactSchema: "agent-utilities/ce-plan-execution-input/v1", roles: ["research", "investigation", "implementation", "implementation.bounded_fix"], carriers: ["glm-5-2-scout", "glm-5-2-engineer", "codex-luna"] }),
  "ce-work.execution": freeze({ skill: "ce-work", artifactSchema: "agent-utilities/ce-work-execution-input/v1", roles: ["implementation", "implementation.bounded_fix", "implementation.mechanical"], carriers: ["glm-5-2-engineer", "codex-luna"] }),
  "ce-debug.execution": freeze({ skill: "ce-debug", artifactSchema: "agent-utilities/ce-debug-diagnosis/v1", roles: ["investigation", "research"], carriers: ["glm-5-2-scout", "codex-luna"] }),
  "ce-code-review.execution": freeze({ skill: "ce-code-review", artifactSchema: "agent-utilities/ce-code-review-findings/v1", roles: ["review.code", "review.cross_family"], carriers: ["claude-ce-review", "oracle-browser"] }),
  "ce-doc-review.execution": freeze({ skill: "ce-doc-review", artifactSchema: "agent-utilities/ce-doc-review-findings/v1", roles: ["review.plan", "review.cross_family"], carriers: ["claude-ce-review", "oracle-browser"] }),
  "ce-pov.execution": freeze({ skill: "ce-pov", artifactSchema: "agent-utilities/ce-pov-review/v1", roles: ["review.cross_family", "review.architecture"], carriers: ["claude-ce-review", "oracle-browser"] }),
  "ce-pr-review.execution": freeze({ skill: "ce-babysit-pr", artifactSchema: "agent-utilities/ce-pr-review-findings/v1", roles: ["review.code", "review.cross_family"], carriers: ["claude-ce-review", "oracle-browser"] }),
});
...
function derivedWorkClassDigest(request) {
  return stableDigest({
    schema: "agent-utilities/work-class/v1",
...
export function resolvePaths({ env = process.env, home = os.homedir(), cwd = process.cwd(), platform = process.platform } = {}) {
  const configOverride = env.AGENT_UTILITIES_MODEL_POLICY_PATH;
  const stateOverride = env.AGENT_UTILITIES_MODEL_STATE_PATH;
  const configPath = configOverride
    ? configOverride
    : platform === "win32"
      ? path.join(env.LOCALAPPDATA || home, "agent-utilities", "model-routing.json")
      : path.join(env.XDG_CONFIG_HOME || path.join(home, ".config"), "agent-utilities", "model-routing.json");
  const statePath = stateOverride
    ? stateOverride
    : platform === "win32"
      ? path.join(env.LOCALAPPDATA || home, "agent-utilities", "state", "model-routing-state.json")
      : path.join(env.XDG_STATE_HOME || path.join(home, ".local", "state"), "agent-utilities", "model-routing-state.json");
...
function presentationOverlayFor({ carrierId, model, effort }) {
  ...
  return base ? {
    schema: "agent-utilities/presentation-overlay/v1",
    family: base.id,
    format: base.format,
    carrierId,
...
function validR28Disclosure(value) {
  ...
  if (value.schema !== "agent-utilities/r28-route-disclosure/v1" || !["selected", "fallback", "settlement"].includes(value.route) || !validId(value.reasonCode)) return false;
...
function validateRequest(input, command) {
  ...
  if (input.callerKind === "machine-utilities" && ["resolve", "admit"].includes(command) && !r52Ready(input.r52)) return error("model_routing_capability_unavailable");
...
function r28RouteDisclosure(candidate, request, {
  ...
  return {
    schema: "agent-utilities/r28-route-disclosure/v1",
    route,
    reasonCode,
    requested,
    configured,
...
function decisionFromCandidate(candidate, request, policy, now, rejected = []) {
  ...
  if (request.ceSeam) {
    ...
    decision.executionOverride = {
      contractVersion: "agent-utilities/ce-execution-override/v1",
...
function fixedOracleRouteRoot(home) {
  return path.join(path.resolve(home), ".local", "state", "agent-utilities", "oracle-route");
}
...
export function runCli(input, options = {}) {
  ...
  const routingEnv = protectedInspection
    ? {
      ...suppliedEnv,
      AGENT_UTILITIES_MODEL_POLICY_PATH: undefined,
      AGENT_UTILITIES_MODEL_STATE_PATH: undefined,
