...
const IMPORTER_ID = "agent-utilities-adapter-receipt-importer-v1";
...
function refreshAttestor({ observedModel, capabilities = [], authState = "authenticated", fallbackSetDigest, expiresAt = "2026-08-04T12:30:00.000Z" } = {}) {
  return (record) => {
    ...
    const result = {
      attestorId: "agent-utilities-host-attestor-v1",
...
test("catalogs, privacy, and closed CE seams cannot widen routing authority", () => {
  ...
  const plan = handleRequest(request("resolve", {
    ...
    ceSeam: { id: "ce-plan.execution", skill: "ce-plan", artifact: { schema: "agent-utilities/ce-plan-execution-input/v1", digest: DIGEST_A } },
  }), { now: NOW });
  ...
  const incompatible = handleRequest(request("resolve", {
    callerKind: "compound-engineering",
    ceSeam: { id: "ce-code-review.execution", skill: "ce-code-review", artifact: { schema: "agent-utilities/ce-code-review-findings/v1", digest: DIGEST_A } },
  }), { now: NOW });
...
test("R28 decision, fallback, and settlement disclosures use explicit provenance without task content", () => {
  const terra = handleRequest(request("resolve"), {
    ...
    trustedRuntimeAttestor: () => ({
      attestorId: "agent-utilities-runtime-attestor-v1",
      attestationDigest: DIGEST_A,
  ...
  assert.equal(terra.response.decision.fallbackReceipt.schema, "agent-utilities/r28-route-disclosure/v1");
...
test("visible bridge acknowledgement and activation bind the exact fixed task identity", () => {
  ...
  const transportAttestor = () => ({
    attestorId: "agent-utilities-transport-attestor-v1",
    attestationDigest: DIGEST_A,
...
test("bounded context forks and R52 readiness use closed content-free wire forms", () => {
  const policy = catalog();
  const readiness = r52Readiness();
  const resolved = handleRequest(request("resolve", { callerKind: "machine-utilities", contextFork: "3", r52: readiness }), { catalog: policy, now: NOW });
  ...
  assert.equal(handleRequest(request("resolve", { callerKind: "machine-utilities" }), { catalog: policy, now: NOW }).response.reason, "model_routing_capability_unavailable");
  for (const key of ["hostReadiness", "taskReadiness", "transportReadiness"]) {
    ...
    assert.equal(handleRequest(request("resolve", { callerKind: "machine-utilities", r52: blocked }), { catalog: policy, now: NOW }).response.reason, "model_routing_capability_unavailable", key);
    assert.equal(handleRequest(request("resolve", { callerKind: "machine-utilities", r52: unknown }), { catalog: policy, now: NOW }).response.reason, "model_routing_capability_unavailable", key);
  }
...
test("the public CLI accepts only a fixed Oracle receipt reference and settles an adapter-emitted private artifact", () => {
  ...
  try {
    ...
    const configDirectory = path.join(home, ".config", "agent-utilities");
    const stateDirectory = path.join(home, ".local", "state", "agent-utilities");
    ...
    const root = path.join(home, ".local", "state", "agent-utilities", "oracle-route");
    ...
    assert.equal(settled.disclosure.schema, "agent-utilities/r28-route-disclosure/v1");
  ...
  try {
    ...
    const nativeSettled = publicCli({
      ...
      receipt: { appToolEvidence: {
        schema: "agent-utilities/app-tool-evidence/v1",
...
test("protected inspect-claim ignores caller path and XDG overrides", () => {
  ...
  try {
    ...
    const inspected = runCli(request("inspect-claim", {
      ...
      cwd: process.cwd(),
      env: {
        AGENT_UTILITIES_MODEL_POLICY_PATH: configPath,
        AGENT_UTILITIES_MODEL_STATE_PATH: statePath,
...
test("state paths fail closed for a selected missing policy and the paired fast-path fixture proves no I/O", () => {
  ...
  try {
    ...
    assert.equal(resolvePaths({ home: directory, cwd: process.cwd(), env: { AGENT_UTILITIES_MODEL_POLICY_PATH: selected } }).ok, true);
    assert.equal(runCli(request("validate"), { cwd: process.cwd(), env: { AGENT_UTILITIES_MODEL_POLICY_PATH: selected, AGENT_UTILITIES_MODEL_STATE_PATH: path.join(directory, "state.json") } }).reason, "selected_policy_missing");
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
