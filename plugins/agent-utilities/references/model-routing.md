...
`agent-utilities:model-routing` is the single public decision point for model,
effort, budget-admission, and dispatch-transport policy. Its exact wire version
is `agent-utilities/model-routing/v1`.
...
`goal-driven-delivery`, `thermos`, and `machine-utilities`. A CE caller must
...
also carries `agent-utilities/r28-route-disclosure/v1`. It gives requested,
...
`agent-utilities/action-receipt/v1` schema. It records the stable action ID and
...
The policy path order is absolute `AGENT_UTILITIES_MODEL_POLICY_PATH`, then
the platform user config path. The state path order is absolute
`AGENT_UTILITIES_MODEL_STATE_PATH`, then the POSIX user state path. Defaults
