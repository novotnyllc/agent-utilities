...
test("blocks an oversized ad hoc script with an actionable message", () => {
  ...
  assert.match(result.stderr, /shipped tooling/);
  assert.match(result.stderr, /bounded feature recipes/);
});
...
test("exempts an oversized shipped enclosure-feature transaction", () => {
  const source =
    "# Generated by fusion-design enclosure-feature boss.heat_set_insert/v1\n" +
    "import adsk.core\n" +
    "value = 1\n".repeat(400);
  const result = run(execute({ script: source }));
