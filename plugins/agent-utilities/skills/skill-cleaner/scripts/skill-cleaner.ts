...
const scriptSkillRoot = path.resolve(path.dirname(scriptFile), "..");
...
function skillRootScope(root: string): string {
  ...
  if (normalized.includes("/agent-utilities/") && normalized.includes("/skills")) return "agent-utilities";
...
function deletePriority(skill: Skill): number {
  ...
  if (skill.path.includes("/.codex/skills/") && !skill.realPath.includes("/agent-scripts/")) return 1;
  ...
  if (skill.realPath.includes("/agent-scripts/skills/")) return 4;
...
function recentLogFiles(): string[] {
  ...
  if (deepLogs) {
    roots.push(
      ...
      ...(process.env.AGENT_UTILITIES_LOG_ROOTS ?? "")
        .split(path.delimiter)
        .map(expandHome)
        .filter(Boolean),
    );
