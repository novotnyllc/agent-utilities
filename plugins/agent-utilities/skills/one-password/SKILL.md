...
metadata: {"agent-utilities":{"emoji":"🔐","requires":{"bins":["op","tmux"]},"install":[{"id":"brew","kind":"brew","formula":"1password-cli","bins":["op"],"label":"Install 1Password CLI (brew)"}]}}
---
...
6. Desktop fallback: before the interactive flow, `unset OP_SERVICE_ACCOUNT_TOKEN` inside the tmux window so an exported service token cannot silently confine reads. Then confirm app integration/unlock and run `op signin` once inside the same session.
7. Verify chosen access path inside that same session: `op whoami`.
8. If multiple accounts: use `--account` or `OP_ACCOUNT`; never `--account` on a service-account path.
...
- Keep package-manager or service-specific item names in the owning task instructions; this skill only supplies safe `op` mechanics.
