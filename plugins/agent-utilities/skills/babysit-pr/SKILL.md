...
7. If the failure is likely caused by the current branch, patch code locally, commit, and push. Do not patch random flaky tests, CI infrastructure, dependency outages, runner issues, or other failures that are unrelated to the branch.
8. If `process_review_comment` is present, inspect surfaced published review items and decide whether to address them.
9. If a review item is actionable and correct, patch code locally, commit, push, and then resolve every associated review thread/comment that the commit addressed, but only when allowed by the GitHub state mutation policy below.
10. Do not post replies to human-authored review comments/threads unless the user explicitly confirms the exact response. If a human review item is non-actionable, already addressed, or not valid, surface the item and recommended response to the user instead of replying on GitHub.
...
6. For each surfaced review item from another author, patch/commit/push if it is actionable, then resolve every addressed eligible thread/comment when allowed by the GitHub state mutation policy above. If it is non-actionable, already addressed, ineligible for automatic resolution, or requires a written answer, surface it to the user with a suggested response instead of posting automatically. If a later snapshot surfaces your own approved reply, treat it as informational and continue without responding again.
...
9. Retry failed checks only when `retry_failed_checks` is present and you are not about to replace the current SHA with a review/CI fix commit. Do not make code changes for unrelated flakes or infrastructure failures just to get CI green.
10. If you pushed a commit, resolved an eligible review thread, or triggered a rerun, report the action briefly and continue polling (do not stop). If a human review comment needs a written GitHub response, stop and ask for confirmation before posting.
