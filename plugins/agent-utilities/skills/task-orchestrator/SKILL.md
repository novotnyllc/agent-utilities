...
invoke `agent-utilities:model-routing` with exact contract
`agent-utilities/model-routing/v1` and consume its immutable snapshot (policy
...
`agent-utilities:model-routing` on every work-starting create, message, or
...
project baseline, saved-project readiness), `machine-utilities:fleet-agents`
(runtimes, plugin versions, skill hashes, capabilities), and
`machine-utilities:fleet-inventory` (read-only snapshot; `fleet-auth` only
