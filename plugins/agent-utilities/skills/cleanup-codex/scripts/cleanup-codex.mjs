...
function strictLauncherPath({ explicit, env, fsApi }) {
  ...
  if (env.AGENT_UTILITIES_CODEX_BIN) return env.AGENT_UTILITIES_CODEX_BIN;
  const candidates = [];
...
function hookStateDirectory(env) {
  if (env.XDG_STATE_HOME) {
    ...
    return path.join(env.XDG_STATE_HOME, "agent-utilities", "cleanup-codex");
  }
  const home = env.HOME || os.homedir();
  if (!path.isAbsolute(home)) throw new Error("hook-state-path-invalid");
  return path.join(home, "Library", "Application Support", "agent-utilities", "cleanup-codex");
}
...
function usage() {
  return [
    ...
    "Unmanaged receipt and recycle also require --unmanaged and --launcher PATH (or AGENT_UTILITIES_CODEX_BIN).",
...
export function runCli(argv = process.argv.slice(2), {
  ...
  if (parsed.action === "recycle") {
    const launcher = parsed.unmanaged
      ...
      : null;
    const attestorPath = parsed.nofileAttestor ?? env.AGENT_UTILITIES_NOFILE_ATTESTOR ?? null;
