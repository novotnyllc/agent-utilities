...
- **Identity is the dataFile id, never the name.** Record the document's dataFile id plus its project and folder ids (in `DESIGN-STATE.md` when the project keeps one). The user may rename the document at any time; later sessions bind by the recorded id and simply report the current name.
