...
The hierarchy is still semantic: the manifest's `references`, `printable_parts`, and `material_decision` blocks are the authoritative classification, and the scaffold writes each component's derived role (`reference`, `packing`, `keepout`, `product`, `validation`) into the `fusion_parametric_design` attribute group as `role`. Machine-readable roles ride on attributes, never on display names. Inventory reads the role attribute first and recognizes the legacy `REF__`/`PACK__`/`KEEP__`/`PROD__`/`FIX__`/`VAL__` name prefixes only as an adoption fallback for documents built before roles were attributes.
