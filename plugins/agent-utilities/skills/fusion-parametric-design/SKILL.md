...
The Fusion document is the product. The timeline, sketches, constraints, components, joints, configurations, and user parameters are the editable source of truth. MCP code is a temporary keystroke, not a program: each snippet is the equivalent of one skilled user action in Fusion.

## 1. Operating rules

These rules are unconditional. They apply to every task this skill handles.

**The lane decision comes first, and ordinary modeling is the default.** The first step of every task is one line of classification: the automation, release, and reconstruction lanes (section 11) activate only on explicit signals — the user asks for a generator, something repeatable, or batch output; requests an export or manufacturing handoff; or asks to reconstruct a mesh into editable CAD. Everything else — design this, model this, change this, fix this, make it look like this — is ordinary interactive modeling, and when in doubt, it is ordinary modeling. Ordinary modeling writes **no files to the user's repository**: no manifest, no `DESIGN-STATE.md`, no transaction scripts, no reports. Fusion holds the design — the document, the timeline, named versions — and the conversation holds the screenshots and decisions. The urge to create a project file structure for a modeling request *is* the misclassification signal; stop and reclassify.
...
Acceptable thin scripting: create one sketch and its dimensions; extrude one profile; edit one feature parameter; create one loft from selected profiles; apply one fillet; insert one linked component; call Fusion Interference; call Fusion Measure; capture one screenshot; read one feature error.

Unacceptable: a large transaction file constructing the complete product; a custom component-tree synchronizer for a one-off enclosure; a custom geometry-verification report; a temporary candidate-generation and rollback system.
...
- `references/add-ins.md` when surveying installed add-ins or reaching for one
...
- **After each coherent batch of changes**: save again. A Fusion save creates a version, and that checkpoint is exactly the desired behavior.
- **Identity is the dataFile id, never the name.** Record the document's dataFile id plus its project and folder ids (in `DESIGN-STATE.md` when the project keeps one). The user may rename the document at any time; later sessions bind by the recorded id and simply report the current name. The id right after a first save can be a local staging path that becomes the stable `urn:` lineage id only after cloud sync — record it provisionally and refresh it from a later checkpoint save before relying on it for reconnection.
- **Reconnecting in a later session**, in order: an *open* document whose dataFile id matches the recorded identity is adopted (never open a second copy); a closed one is located by id through Fusion's data API and opened; with no recorded identity, fall back to create-or-adopt above. A recorded id that cannot be found is a named refusal reporting what was recorded and what was findable — never adopt by name alone.
...
Before the first mutation, confirm that the active product is a Fusion Design, that the design is parametric with design history enabled, and capture an initial view when the MCP supports it. Never switch a populated design from parametric to direct mode — that destroys design history. If the document is direct, stop and recommend a new parametric document or a deliberate manual conversion plan.
...
### Represent every real object usefully

For a manufactured part, cable interface, or mating object, maintain the representations that answer different questions:

- **Editable reference model** — role `reference`: a simple Fusion-native parametric model exposing the planes, axes, mounting holes, connector centers, support faces, and datum geometry needed to author the product. For a cataloged purchased part, the canonical linked component *is* this model.
...
- **Functional keep-outs** — role `keepout`: separate solids for space the object needs but does not occupy at rest — connector insertion and extraction, cable overmold and straight departure, bend radius and service loop, terminal tool access, button travel, fastener driver access, antenna/RF exclusion, acoustic paths, ventilation, assembly insertion, lid travel. The product may touch intended support surfaces; it may not intrude into a keep-out. When a reference genuinely has no such envelope, record a specific rationale.
...
For an enclosure or packed product: place all packing components in installed position, place all keep-outs, define support faces, mounting datums, and insertion order, settle wire and cable routes and service access, run minimum-distance and interference checks — and only then establish the internal boundary and exterior form. Do not shrink the shell by deleting service space; when a package is too large, reconsider orientation, shared clearance regions, connector direction, lid split, or component choice, and state explicitly when a requested envelope is physically incompatible with the recorded packing.

**Joinery is proposed proactively, and always as real geometry.** Wherever parts meet — a lid on a base, a bracket on a housing — and wherever a single part would print better split (a raised ring on a flat surface often prints better as two parts rejoined), the expert move is to surface the join recommendation with its reason: snap fit (cantilever or annular), screws with captive nuts, press fit, heat-set inserts, keys and dovetails — chosen for the materials, the loads, and the open/close or reuse cycle, with 3D-printability an explicit driver (build orientation, support elimination, seam placement, and layer-direction strength can each justify splitting a part and picking the join). A suggested join arrives modeled or immediately modelable, never as prose only: fasteners are real cataloged components per section 4 — Insert Fastener hardware, captive nuts as catalog parts — positioned with joints and joint origins so native Interference and Measure validate engagement and clearances for real. When the user's hub holds prior designs with established joining patterns, read them through the Data Panel and reuse the house style — reuse beats invention. Suggestions ride the normal visible-result loop: offered when the geometry raises them, never as a blocking analysis phase, and dropped without ceremony when the user picks otherwise. The material gate in section 5 applies to every material-dependent join before it is settled.

When the design is built around electrical modules, wiring is part of the design: wire runs are native geometry — a 3D sketch path swept with Pipe or Sweep at the run's overall outside diameter (insulation and jacket included, never bare-conductor gauge), in real wire colors, with the Wire Generator add-in preferred when installed — terminations (ferrules, fork and ring terminals, connector housings) are cataloged components placed at wire ends, and gauge, conductor count, voltage, and current per run are recorded in base-tier descriptions and attributes. Recorded data and gentle advisories only — never a host-side electrical-validation engine. Base Fusion has no general harness environment and the Electronics workspace is PCB design, so wire modeling is ordinary sweep/pipe modeling with recorded metadata, and wires appear when they matter to the design (`references/enclosure-workflow.md` § Wiring and terminations).

Design the enclosure as an assembly, not two isolated solids: base, lid, fasteners, buttons, gaskets, cable exits, and internal parts present together, with the installed electronics and all keep-outs, seam and printed fit, fastener engagement and tool access, connector insertion and removal, wire routing, button travel, removal sequence, and body-facing smoothness for wearables all checked. For moving mechanisms, create Fusion joints and limits; a motion check samples the joint parameter across the declared range, recomputes, and runs interference at each sample — a static open and closed view is not a substitute when intermediate motion can collide.
...
Ordinary Fusion modeling is one agent operating Fusion. No review agents before a visible result. A specialist may be consulted only for a narrow missing Fusion capability or a documented API question, and its advice must lead directly to one native Fusion action. A simple modeling task is never an agent-orchestration exercise. Dispatch shape and model tiers: `references/model-routing.md`.
...
## 11. Automation and release lanes

The machinery in this section — the project manifest, generated transactions, verification contracts, deterministic export, the PrusaSlicer adapter, variant matrices, and mesh reconstruction — exists for four situations:

- the user explicitly requests reusable automation, batch generation, or a repeatable product generator;
- the user requests a manufacturing export or handoff;
- the task is reconstruction of a scanned or downloaded mesh;
- the project's existing conventions already keep a manifest and evidence chain.

It is never invoked for ordinary modeling or visual edits. When a lane applies, its tools are the required path — a manufacturing export goes through the verification-bound export transaction, not manual export selection.
