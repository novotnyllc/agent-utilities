...
Ordinary modeling normally does not use generated transaction contracts. A
supported single-feature request may invoke the shipped bounded recipe below
without leaving the ordinary lane; developing or modifying the recipe remains
a separate automation software task.
...
### Bounded enclosure-feature transaction

`fusion_design.enclosure_features` emits one create/edit/inspect/delete
transaction for one managed feature instance. It uses the
`# Generated by fusion-design` header, prints the smaller
`FUSION_ENCLOSURE_FEATURE_RESULT_*` payload, and never uses the release report
protocol or writes a host artifact. Create requires explicit entity tokens for
the target body, support face, and placement point; modifying feature inputs
must set participant bodies explicitly. Lifecycle operations accept only the
managed tokens/parameters returned by creation and refuse an unrecognizable
manual edit. The transaction may contain reusable helpers because it is shipped
versioned tooling, but it may not search geometry, generate alternatives,
retry, validate a product, or create more than one feature instance.
