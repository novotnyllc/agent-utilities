...
| Mesh Section Sketch / Fit Curves to Mesh Section | No API equivalent exists. Section polylines are computed from `PolygonMesh` node coordinates and triangle indices, and classified into lines and arcs in `mesh_fitting.py` | **UI-only in Fusion**; no emitted script may call them. Sketch API emission from those fits *is* built — `emit-mesh-rebuild` creates the sketch, the constraints and the dimensions itself — but never through these two UI commands |
| Scan-to-parametric archetypes | `plan-reconstruction` assigns each accepted fit to `sketch-extrude`, `revolve`, `hole` or `fillet`; `emit-mesh-rebuild` builds all four in one data-driven transaction | Supported, each under stated conditions. A hole needs `orientation.material_side == "inside"`, which needs a watertight mesh; a fillet needs both its neighbours rebuilt. Anything else is unreconstructed with a named gate, never approximated |
| Reconstruction coverage report | `reconstruction-coverage` composes the fit record, program, rebuild report and editability verdict into one labelled account | Supported. Labels are the closed set `parametric-full` / `parametric-partial` / `reconstruction-refused`; the delivered fraction subtracts every planned archetype the build did not deliver |
| Editability proof | `emit-mesh-editability` perturbs each parameter against its declared observable and restores it; `check-editability` is the offline gate | Supported. It never emits a `parametric: true` boolean and always reports `interactions_exercised: false` |
