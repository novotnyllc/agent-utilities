...
`extent_expression` is the design's station model written down — a slab's depth is the difference of two station parameters, so editing one boundary moves it and everything referencing it, and no depth is stored twice. It is **recorded, not bound**, here: the emitter still drives each extrude from its own depth parameter, and minting station parameters nothing in the timeline references would emit parameters that drive nothing and fail the perturbation proof by construction.
