...
**The emitter does not build slab stacks.** A multi-slab program refuses `program-schema-violation` naming the slab count: it needs one sketch per slab holding several loops and a profile-set resolver in place of the largest-area rule, and building each slab's outer loop while dropping its cavities would be the improvisation this package bans. A part that decomposes to **one** slab plans exactly what the planner planned before slabs existed — same regions, same caps, same extent — with the slab record beside it, which is what makes the generalisation safe.
