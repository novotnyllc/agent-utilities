...
**Omission accounts for some of those samples, and the verdict checks whether it does.** When the rebuild leaves out an outward feature the scan carries, the surface it leaves in that feature's place is a rebuilt surface far from any scanned triangle — on a manifold scan there are no base-face triangles under a fused boss for it to be near — so the reverse direction reads exactly what invented material reads. The signed direction already saw that omission, as scanned vertices *outside* the reconstruction. So the verdict compares reaches: when the reconstruction-to-source maximum is no further than the omitted-detail maximum, the omission is taken as the account of it, `unclassified_explained_by_omission` records that with both numbers, and the run passes with the omitted-detail advisory it should have had. When it reaches further, nothing explains it and the run fails closed. What that does **not** establish is the absence of invented material *as well* — a part carrying both, with the invention lying between scanned vertices, looks like this — and the recorded meaning says so. Classifying those samples against a closed source solid is what would settle it.
