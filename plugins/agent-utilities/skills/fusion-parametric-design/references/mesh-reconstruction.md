...
A converted body is **faceted**, never parametric, and must never be reported as parametric — and it is never a reconstruction result. At most, per-region converted faces serve as *candidate fits*, judged by the same disproof gates as native fits, with their provenance recorded; the reconstruction itself is always an emitted feature program.
...
| Dimensional or structural change; a boolean on a mesh that is not watertight or is over budget; or the scanned object itself is wanted as editable CAD | `parametric-rebuild` |
...
- **`parametric-rebuild`** — whole-part editable reconstruction is the goal: recognized surfaces become real sketches, constraints, dimensions, and timeline features for as many regions as the evidence licenses, with partial coverage a first-class outcome (below) — every unreconstructed region named with its gate, never silently absorbed. An edit-scoped rebuild (only the geometry one edit requires) is the same pipeline stopped early, not a different product. Whole-part is the goal; wholesale *conversion* is not the method — geometry is claimed by evidence and refused by name. Near-coaxial, near-perpendicular, near-symmetric, and near-nominal values are surfaced as proposals carrying their deviation, never snapped silently.
...
Inside Fusion, first: `emit-mesh-face-groups` runs `MeshGenerateFaceGroups` on the mesh body with `AccurateGenerateFaceGroupsType` set explicitly, and `emit-mesh-extract` then writes the hash-bound dump carrying that grouping, one id per triangle. Host-side, with no Fusion running: `fit-regions` builds the region tree over that grouping and fits an analytic primitive to each of its terminal regions behind the disproof gates. **The tree's splitter is dispatched on the dump's own regime verdict**, so the stage that answers the mega-group is inside `fit-regions` rather than beside it: a tessellation-regime dump's terminals are Fusion's groups unchanged, and a scan-regime capture's groups are refined by the scan segmentation below — a region tree whose splits are licensed by fit rejections, never by size — before any fit is judged. A capture whose segmentation recovers no orthogonal frame to split against is refused `segmentation-datum-unavailable` and flows to the single-body path rather than being split by a rule nobody measured, and a region the splitter reaches no licensed split on stays a terminal carrying the gate that stopped it. Then `plan-reconstruction` derives a datum frame from the accepted fits and assigns them to archetypes; `reconstruction-coverage` composes the final account. Back inside Fusion: `emit-mesh-rebuild` builds the timeline, `emit-mesh-editability` proves each parameter drives it, and `emit-mesh-deviation` grades the result against the immutable source.
...
So Fusion's grouping is the first candidate partition — never the authority. On tessellation-regime dumps it is routinely sufficient and the fitters consume it directly, which is what every number in the table above measures. On scan-regime captures it is measured insufficient — one group can hold most of a capture's area — so the pipeline's own scan segmentation (a region tree whose splits are licensed by fit rejections, never by size) refines the grouping there; the gates judge every region the same way whichever source produced it, and the tables above are tessellation-regime numbers. What survives untouched is the part Fusion has no opinion about: support floors, Moran's I on the mesh graph, the spatially blocked held-out refit, the nested-kind model comparison (next paragraph), and the parameter-uncertainty gate all still run on every group, and a group that fails one is recorded with the gate that killed it. *Run* is not assumed: both structure gates have a power floor — against residuals an order of magnitude inside the measurement noise they have nothing to test, and they say so instead of passing — so `disproof.gates` counts, per gate, how many accepted fits it actually judged and why it skipped the rest, derived from each region's own `checked` list rather than asserted beside it. On the honeycomb organiser that floor skipped Moran and the held-out refit on all 39 accepted planes. That gap between 1,069 fitted and 268 accepted -- the table's *left* column, before the normals became fit data; 619 in the right column and 598 after the prism filter -- is the gates doing their job, not a loss: `fit_primitive` alone accepts nearly everything, and the disproof gates are the difference between a fit and a *justified* fit. A tessellation-regime dump that carries no grouping is refused `face-groups-absent` rather than segmented by a fallback nobody measured; a scan-regime capture routes to the scan segmentation, whose own refusals are named.

**Nested kinds are compared out of sample, never by a classical F statistic.** Cylinder-in-cone (ω = 0), cylinder-in-torus (R → ∞), and sphere-in-torus (R = 0) are singular or boundary limits — the parameter leaves the space or sits on a boundary where identifiability changes, so the finite-sample F distribution is not licensed merely by writing one kind as the other's limit. The comparison runs, in precedence order: (1) **spatially blocked held-out comparison** — both kinds fitted on the same non-validation blocks, held-out RMS compared, the richer kind kept only when its advantage exceeds the caller-declared `nested_kind_heldout_margin`; (2) where held-out support is too thin to be decisive, a **parametric bootstrap under the simpler kind** — replicates simulated from the fitted simpler model plus the licensed noise model, both kinds fitted to each replicate, the observed richer-kind improvement judged against the simulated null, replicate count and seed recorded. The verdict is recorded as **model-selection evidence** — both scores, the margin, the method used — never as an exact classical p-value whose assumptions are unverified; where neither path is licensed the comparison fails closed to the simpler kind, recorded `nested-kind-comparison-unlicensed`. The covariance-triggered demotions (`torus-demoted-cylinder`, `torus-demoted-sphere`, `cone-unidentifiable`) are separate and unchanged: they fire on identifiability, which is where they belong.
...
| `dirty_wall_triangles` | the dump triangles among this loop's own walls that sit on a non-manifold edge. Empty is the normal case; non-empty is the whole reason this loop refuses `loop-orientation-unavailable` while its neighbours on the same section do not |
...
A self-touch can also be two *shells* meeting at an edge, and one global sign is a claim about one solid: two closed shells wound opposite ways share no boundary edge, so the total signed volume takes the larger one's and every clean loop on the smaller is flipped. So the surface is separated into shells over its manifold edges and their signs compared — and where a non-manifold edge joins shells that disagree, the mesh carries no global winding at all. A solid with an internal *void* is also two shells of opposite sign, but they are disjoint, so it keeps its winding, which is why the comparison is conditioned on there being a non-manifold edge in the first place.

The licence is **per edge, and the two kinds of dirt are not the same fact**. A *boundary* edge — one incident triangle — means the surface has a hole in it, so it encloses no volume, the sign of the signed volume means nothing anywhere, and no loop at any station can be classified. A *non-manifold* edge — three or more — is the surface touching itself: it tears nothing, the body still encloses its volume, and the direction stands. `non_manifold_triangles` names exactly the triangles on that dirt, and a loop refuses `loop-orientation-unavailable` only when its **own** walls are among them.

That distinction was measured, not assumed. Under a whole-mesh licence, two non-manifold edges in 10,200 triangles cost POD-A2-LID every loop at every station, and one edge in 86,394 cost the tropical leaves both of theirs — neither part has a single boundary edge. Per edge, thirteen of those fifteen loops classify at consensus 1.000, and the one that still refuses is the one whose walls sit on the bad edge.

### Events, slabs, and the 2.5D decomposition

`plan-reconstruction` decomposes a part along the datum primary axis when the caller declares `thresholds.slab_evidence` **and** passes `--dump`. Both are required and neither is defaulted: a slab's loops are cut from the dump's own triangles, which the fit record does not carry, and undeclared gates mean no claim is made — the same rule the kinematic router already follows. Without either, the plan is exactly what it was before slabs existed and `program.slab_decomposition` says which was missing.

**Events** are the stations where the cross-section's topology can change. Every accepted plane whose normal lies on the axis contributes one at its fitted station carrying its own `offset` sigma (`plane-fit`, the *defining* evidence); every side region's axial span end contributes a `span-end` corroboration carrying that region's own positional sigma, because a wall ends where some face bounds it and that face may be one no fitter accepted. They merge by **complete linkage** — a station joins the open cluster only while it is within `event_merge_sigmas × √(σᵢ² + σ_first²)` of the cluster's *first* member, never its last, because linking to the last is single linkage and single linkage smears a row of real stations into one. The merged station is the inverse-variance weighted mean, so a fitted plane dominates the bounding-box corroborations that agree with it.

Every member's sigma is floored at the declared `section_tolerance_mm`, and that floor is load-bearing: a plane fitted through analytically planar points — which is what every one of these parts is — reports an offset sigma of **zero**, so the derived tolerance collapses to zero and nothing merges with anything. Measured on POD-C-LID that produced 76 event stations and a slab nine microns thick. A zero sigma is not a claim of infinite precision; it is the fitter saying the residual is below what it can see, and two stations closer than the tolerance this stage sections at are not two stations it can tell apart. With the floor, POD-C-LID is 5 events and 4 slabs.

**Slabs** are what lies between consecutive events, one join-only extrude each: slab 0 is `new-body` and every slab above it is `join`, so the stack is one solid built by several named features. Each is sectioned at its midpoint *and* at the declared `slab_section_fractions` of its own height, and those sections must close the same loops in the same places (symmetric point-to-polyline Hausdorff within `slab_constancy_tolerance_mm`, matched by nearest centroid). **Those stations are the seed set, not the certificate.** A fixed handful of stations cannot prove that no topology change lies between them: a narrow boss, slot, rib, chamfer termination or scan defect can live entirely inside one interval, and the event machinery does not close that gap, because an event exists only where a bounding plane was accepted or a side-region endpoint measured — and a feature that hides between stations is exactly one whose planes the fitters refused. So certification terminates on evidence rather than on a station count. Each interval's triangle support is compared against what the fitted side surfaces and the event hypotheses predict for it, and a further section is inserted only where that support is **unexplained or contradicts the prediction**: side-region residual structure exceeding the fitted model's bound over a sub-interval, triangles no fitted side surface explains, or a side-region endpoint inside the interval. Model-consistent unsectioned support triggers nothing — a constant slab is certified by its side surfaces' own residual bound, not by sectioning every axial triangle row, or the loop would degenerate to one section per row and exhaust its budget refusing valid slabs. Each inserted station is the quantized location of the evidence that demanded it, so two runs section the same stations. The loop repeats until every interval is certified within `slab_constancy_tolerance_mm`, or the slab refuses `slab-section-inconstant` naming the interval and the unresolved evidence — which is where a tapered wall honestly lands until lofts exist. `slab_certification_max_sections` caps it, caller-declared with its rationale like every other threshold; exhausting the budget is that same refusal and never a silent pass, and the certification record carries every station sectioned, the evidence that demanded it, and the bound each interval reached. An event whose two adjacent slabs section to congruent loop sets bounds no topology change, so it is demoted to corroboration and the slabs merge, recorded on the event rather than dropped.

**Loops** are classified from PR-1's winding evidence crossed with even-odd nesting: depth-even and material-inside is `outer` (depth 0) or `island`; depth-odd and material-outside is a `bore` when every fitted wall belongs to a bore region this program already cuts, and a `cavity` otherwise. A bore is identified by its walls' **region identity**, not by matching a declared centre and diameter — at plan time the bore regions are known by hash, and identity is stronger evidence than a centre agreeing to a tolerance. A loop whose evidence reached no material verdict is `unclassified`; the slab records it and is not built.

`sketch-extrude` archetypes gain a `slab` block (`index`, `of`, `lower_event`, `upper_event`, `station_parameters`, `extent_expression`, `relation_to_below`, `section_station`, `constancy`, `winding`, `loops`, `gates`) and the program header gains `events` and `slab_decomposition`. **`PROGRAM_VERSION` is 2**, and a v1 program is refused rather than best-efforted.

`extent_expression` is the design's station model written down — a slab's depth is the difference of two station parameters, so editing one boundary moves it and everything referencing it, and no depth is stored twice. When the planner landed it, it was **recorded, not bound**: the emitter of that unit drove each extrude from a depth parameter of its own, and minting station parameters nothing in the timeline referenced would have emitted parameters that drive nothing. **The emitter binds it now** — see "Emitting a slab stack" below — so a slab stack has no per-slab depth parameter at all, and the seam test asserts that none exists.

Region assignment is a **partition** — each region goes to the slab whose centre is nearest the midpoint of the region's own axial span — because the coverage account sums what each archetype claims and a wall counted by three slabs would inflate it. Nearest centre, not containment: the two agree while the slabs are equal in height, and where they differ a region inside a tall slab can be nearest a shorter neighbour's centre and is claimed by that neighbour. Both rules partition, which is all the account needs. A **slab stack counts as one base** for hole planning: the "which body is this bore in?" question the single-base rule exists to refuse does not arise for a joined stack, so the span is the union of its slabs' and the bore is ordered after the last it reaches into. That union is a true span only while the stack is continuous, so a slab gated out from *between* two surviving slabs refuses the decomposition (`slab-stack-discontinuous`) and leaves the single extrude standing: a `join` across the gap would fuse nothing, and a bore lying inside the gap would read as contained by a stack that is not there.

A part that decomposes to **one** slab plans exactly what the planner planned before slabs existed — same regions, same caps, same extent, same parameters — with the slab record beside it, which is what makes the generalisation safe. It also *emits* exactly what it emitted: same plane, same extent, same entities, same constraint schedule, same dimension set.

### Emitting a slab stack

One sketch per slab, on a construction plane offset from an origin plane by that slab's lower **station parameter** — never `setByPlane`, and never a per-archetype offset of its own, so the two slabs meeting at a boundary name the same number. The extent is the two-station expression `recon_station_{k+1} - recon_station_{k}`, so no boundary is stored twice and editing a station moves that face and every slab referencing it. Slab 0 is `new-body`, the rest `join`, in dependency order.

Each sketch holds every loop the slab's section closed **except the bores**: a bore is a hole feature this same program cuts, ordered after the extrude, and drawing it here as well would remove its material twice. Every entity row carries the `loop` it belongs to, and each loop chains and refuses closure on its own — a gap in one loop is that loop's gap, not the sketch's.

**How Fusion enumerates a multi-loop sketch was measured, not assumed.** A live probe (Fusion 2.0.23164) on a sketch of four rectangles — an outer square, two islands in it, and a fourth square nested inside the first island — returned **four** profiles, not two: one per loop, each being that loop minus its immediate children, *regardless of nesting parity*. Fusion reports the planar subdivision; it has no opinion about which regions are holes. `mesh_slabs.profile_regions` computes exactly that set host-side, flags the even-depth regions as `material`, and the areas it predicts (92, 3, 4, 1 cm²) are the areas Fusion returned. It is computed from the **classified entities**, not from the section's chords: a regular octagon's vertices lie exactly on its circumscribed circle, so `classify_polyline` turns it into that circle with no residual at all, and the circle encloses about 10% more than the chords do. A region measured on the chords and built as an arc would be a `profile-set-mismatch` on geometry Fusion built exactly as classified, and the classification residual does not bound that difference. Containment is still read from the chords, because an arc and its chords enclose the same loops.

The executor's `_profile_set` then matches Fusion's own enumeration against the plan's by centroid **and** area, extrudes the regions flagged `material` as one `ObjectCollection`, and refuses `profile-set-mismatch` on any zero-or-multiple match or on a profile the plan cannot name — a profile left unaccounted for is geometry the transaction would silently leave out of the feature. That retires `_profile`'s largest-area rule for slab steps, which was itself the positional choice the rest of the executor bans. The largest-area rule stays for a single-loop sketch, where there is one region and nothing to choose between.

**The match allows for the geometry the transaction itself moved.** The plan's regions are computed from the *section*; every constraint the executor then accepts is licensed to move the sketch by up to the declared `constraint_displacement_tolerance_mm`, so Fusion is enumerating profiles of a contour that has legitimately changed since the plan measured it. The area that moves with a displaced boundary is the displacement times the boundary's own **length**, which scales with perimeter and not with the square root of area — on a long thin region those differ by an order of magnitude, and a window scaled only by the latter refuses a profile set the transaction asked for. So each planned region carries its own `perimeter_cm`, and the match window is `entity_match_tolerance * max(1, sqrt(area)) + displacement * perimeter` on area and `entity_match_tolerance + displacement * (1 + perimeter * extent / area)` on the centroid. The centroid needs that second factor because it is governed by the changed first *moment*, not by how far any point moved: an accepted snap that shifts a small part of a long thin boundary keeps every sketch point inside the displacement and swings the centroid by the added area — `displacement * perimeter` of it — at a lever of up to the region's own extent from its centroid, which each planned region carries. And `displacement` is the larger of the declared tolerance and the **measured** `profile_displacement_mm` — the distance the geometry actually ended up from where the section put it. It has to be the measured one: each constraint and dimension is accepted against the declared tolerance on *its own* movement, and several accepted ones accumulate, so a window of one tolerance refuses a contour this transaction licensed step by step. Nothing widens past what was accepted: an unresolvable displacement (the point count changed) is a different fault and grants no allowance at all.

**Which observable a station moves is measured per station, not assumed.** A station bounds a slab, so an *outermost* one changes that slab's height and the volume with it. An *interior* one grows one slab and shrinks the other, and the volume moves only if the two sections differ in area — coalescing establishes that adjacent sections are not *congruent*, which is weaker: two different shapes of the same area leave the total volume exactly unchanged, and declaring `volume` there would make the perturbation proof report a correct station `parameter-inert`. So the planner compares the two sections' measured material areas (outer and islands adding, cavities subtracting; bores excluded because their own hole features cut them later) and declares `volume` when they differ and `centroid` when they do not, with the two numbers in the rationale. **The comparison carries a window**, and that window is the point of it: these are floating-point sums over a tessellated section, so an exact comparison is true for one part in 10¹⁵ and would declare `volume` on every station, leaving the `centroid` branch unreachable on measured geometry. The window is `slab_constancy_tolerance_mm × max(perimeter, 1)` — the area a boundary displaced by that tolerance sweeps, which is the same number and the same reasoning this stage already uses to call two sections of one slab the same section. Where the two shapes also share a centroid, no observable in this vocabulary moves and the record says so rather than claiming one.

Two other live-probe results the emission rests on: a compound `createByString` extent over **two** user parameters builds and stays live under a parameter edit (assumption S2 — confirmed, so the design's per-slab-depth fallback is not needed), and a construction plane at offset **zero** from an origin plane is legal, which is what lets the bottom slab's plane be a station like every other.

**`sketch_loop_budget`** is a declared runtime guard, and the probe says plainly that it is not a solver limit: profile enumeration was exact and every extrude healthy up to **192** loops in one sketch. What grows is the cost of *drawing* the sketch through the API — 0.048 s at 8 loops, 1.46 s at 64, 14.3 s at 192, roughly quadratic — so the budget exists so that a 200-loop sketch is a decision rather than a hang. `sketch-loop-budget-exceeded` names it.
...
A boss modelled as an extrude and the same boss modelled as a revolve are indistinguishable from the outside. The archetype precedence is therefore a **proposal order**, not a selection authority: it fixes the deterministic order in which earned archetypes are tried and recorded — revolve before sketch-extrude, bores classified before the extrude group chooses its side surfaces — and where only one candidate is licensed, proposal order is all it ever needs to be. Where two materially different archetypes are both licensed by their own evidence and both reproduce the observed surface within the same fidelity budget, neither wins by being tried first: both candidates are constructed and compared on the selection gates in their declared order (semantic contradictions, held-out fidelity, unexplained support, unsupported inference, compactness), and a tie surviving every gate is a **named ambiguity** — refused `program-selection-ambiguous` with the tied candidates and their gate vectors on record, or resolved only under the caller's declared `program_selection: resolve-by-order` policy, recorded as a policy resolution and never presented as evidence-preferred. The result is a feature tree consistent with the measured surface. It is not a claim about what the original designer did, and nothing in the pipeline reports one. The deliverable has a name: an **evidence-licensed editable design-intent hypothesis** — compact, consistent with the observed geometry and the declared priors, and proven editable by perturbation. Never "the original design intent".
...
- **Base features never hold reconstructed geometry — or stand-ins for it.** A base-feature body recomputes without responding to any parameter: history-free geometry wearing a timeline item. Regions the editable ladder cannot claim stay reference mesh and are counted unreconstructed; they are never surfaced into a base feature, labelled or otherwise.
- **Whole-part *auto-conversion* — converting the mesh wholesale and calling the result reconstruction — stays out of scope; whole-part reconstruction *by evidence* is the goal, and the difference is every gate in this reference.** Organic/freeform surface recovery and automatic design-intent assertion remain out of scope. See `references/unsupported.md`.
