...
Region assignment is a **partition** — each region goes to the one slab containing the midpoint of its own axial span — because the coverage account sums what each archetype claims and a wall counted by three slabs would inflate it. A **slab stack counts as one base** for hole planning: the "which body is this bore in?" question the single-base rule exists to refuse does not arise for a joined stack, so the span is the union of its slabs' and the bore is ordered after the last it reaches into.
