...
**Slabs** are what lies between consecutive events, one join-only extrude each: slab 0 is `new-body` and every slab above it is `join`, so the stack is one solid built by several named features. Each is sectioned at its midpoint *and* at the declared `slab_section_fractions` of its own height, and the three must close the same loops in the same places (symmetric point-to-polyline Hausdorff within `slab_constancy_tolerance_mm`, matched by nearest centroid) or the slab refuses `slab-section-inconstant` naming the measured disagreement — which is where a tapered wall honestly lands until lofts exist. An event whose two adjacent slabs section to congruent loop sets bounds no topology change, so it is demoted to corroboration and the slabs merge, recorded on the event rather than dropped.
