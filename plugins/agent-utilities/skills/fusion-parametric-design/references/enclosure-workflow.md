...
When the shipped enclosure feature toolkit is installed, its recipes are the
default implementation for the recurring operations below — bosses and their
hardware, seams (lip/groove, skirt/channel), snap retention, supports, cutouts,
and joinery — invoked as one coherent managed feature group through the add-in
(`references/enclosure-features.md`). The toolkit changes *how* these features
are built, not what the design must satisfy: packing keeps precedence over
feature convenience, load paths keep precedence over geometry shortcuts, and a
recipe that cannot honor the packing solution is refused rather than nudged.
Coupons generated by the toolkit follow `references/verification-contract.md`;
their results follow `references/enclosure-feature-rules.md`.
