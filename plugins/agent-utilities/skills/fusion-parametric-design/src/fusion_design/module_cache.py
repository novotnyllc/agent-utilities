...
#
# The bytes that are hashed are the bytes that are executed. Verification reads
# each module through an O_NOFOLLOW descriptor and keeps the contents; the import
# is then served from those in-memory contents by a meta-path finder, so nothing
# re-opens the cache by path after it was checked. Re-opening by path is what let
# a writer to the cache swap a module in the window between verification and
# import and have the swapped code run under a verified-looking bundle.
import hashlib
...
def _require_private(info, path):
    if info.st_uid != os.getuid():
        raise RuntimeError("cached module bundle is not owned by the current user: " + path)
    if stat.S_IMODE(info.st_mode) & 0o077:
        raise RuntimeError("cached module bundle is accessible by group or other users: " + path)


def _verify_package(bundle_dir, package_name, expected_hashes):
    """Return {{relative path: source bytes}} for a bundle that verifies, else raise."""
    if set(os.listdir(bundle_dir)) != {{"bundle.json", package_name}}:
        raise RuntimeError("cached module bundle contains unexpected entries")
    _require_private(os.stat(os.path.dirname(bundle_dir)), os.path.dirname(bundle_dir))
    _require_private(os.stat(bundle_dir), bundle_dir)
    ...
    for directory, names, filenames in os.walk(package_dir, followlinks=False):
        ...
        _require_private(os.stat(directory), directory)
        for name in names:
        ...
        for name in filenames:
            ...
            actual_hashes[relative] = hashlib.sha256(content).hexdigest()
    if actual_hashes != expected_hashes:
        raise RuntimeError("cached module package failed runtime verification")
    return sources


class _VerifiedFinder(object):
    """Serves imports from verified bytes; never touches the filesystem again."""

    def __init__(self, package_name, package_dir, sources):
        self._modules = {{}}
        for relative, content in sources.items():
            ...
            if is_package:
                parts = parts[:-1]
            fullname = ".".join([package_name] + parts)
            self._modules[fullname] = (content, os.path.join(package_dir, relative), is_package)
    ...
    def find_spec(self, fullname, path=None, target=None):
        ...
        return importlib.util.spec_from_loader(
            fullname, self, origin=entry[1], is_package=entry[2]
        )
    ...
    def exec_module(self, module):
        content, origin, _ = self._modules[module.__name__]
        exec(compile(content, origin, "exec"), module.__dict__)
...
def run(context):
    ...
    try:
        ...
        sources = _verify_package(bundle_dir, package_name, expected_hashes)
        finder = _VerifiedFinder(package_name, os.path.join(bundle_dir, package_name), sources)
